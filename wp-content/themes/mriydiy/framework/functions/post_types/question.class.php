...
class Question {
    ...
    public function register_post_type() {
        $labels = array(
            'name'               => __( 'Питання', \PS::$theme_name ),
            'add_new'            => __( 'Додати питання', \PS::$theme_name ),
            'new_item'           => __( 'Нове питання', \PS::$theme_name )
        );
    ...
    public function count_new_post() {
        global $menu, $wpdb;
        $count = $wpdb->get_var( "SELECT COUNT(ID) FROM {$wpdb->posts} WHERE post_type = 'question' and post_status = 'publish'" );
        foreach ($menu as $key => $item) {
            if ($item[2] == 'edit.php?post_type=question') {
