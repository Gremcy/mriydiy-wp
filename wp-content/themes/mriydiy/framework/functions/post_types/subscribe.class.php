...
class Subscribe {
    ...
    public function register_post_type() {
        $labels = array(
            'name'               => __( 'Підписка', \PS::$theme_name ),
            'add_new'            => __( 'Додати e-mail', \PS::$theme_name ),
            'new_item'           => __( 'Новий e-mail', \PS::$theme_name )
        );
    ...
    public function count_new_post() {
        global $menu, $wpdb;
        $count = $wpdb->get_var( "SELECT COUNT(ID) FROM {$wpdb->posts} WHERE post_type = 'subscribe' and post_status = 'publish'" );
        foreach ($menu as $key => $item) {
            if ($item[2] == 'edit.php?post_type=subscribe') {
